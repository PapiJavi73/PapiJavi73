...
// ── Config reutilizable de Quagga ──
function refQuaggaConfig(videoConstraintsExtra) {
  const constraints = {
    width:  { min: 640, ideal: 1920, max: 1920 },
    height: { min: 480, ideal: 1080, max: 1080 },
    aspectRatio: { ideal: 1.7777 }
  };
  if (videoConstraintsExtra && videoConstraintsExtra.deviceId) {
    constraints.deviceId = videoConstraintsExtra.deviceId;
  } else {
...
// ── Enlazar detectores de Quagga ──
function refBindDetectores() {
  Quagga.offProcessed();
  Quagga.offDetected();
  ...
  Quagga.onProcessed(result => {
    ...
    if (result && result.codeResult && result.box) {
      Quagga.ImageDebug.drawPath(result.box, { x: 0, y: 1 }, ctx, { color: '#00C853', lineWidth: 3 });
    }
  });
...
async function refToggleScanner() {
  ...
  Quagga.init(refQuaggaConfig(), err => {
    if (err) {
...
let refZoomMin      = 1;
let refZoomMax      = 1;
let refAutofocoOn   = true;   // toggle de enfoque continuo
let refCamaras      = [];     // lista de cámaras traseras disponibles
let refCamaraIdx    = 0;      // índice de la cámara en uso

async function refConfigurarCamara() {
  try {
    ...
    // Guardar rango de zoom real del dispositivo
    if (caps.zoom) {
      refZoomMin    = caps.zoom.min || 1;
      refZoomMax    = caps.zoom.max || 1;
      // Zoom inicial suave: 25% del recorrido disponible
      refZoomActual = refZoomMin + (refZoomMax - refZoomMin) * 0.25;
      await track.applyConstraints({ advanced: [{ zoom: refZoomActual }] }).catch(() => {});
      refActualizarZoomLabel();
    } else {
      // Sin zoom nativo: ocultar botones
      const zm = document.getElementById('ref-zoom-controls');
      if (zm) zm.style.opacity = '0.35';
    }

    // Autofoco continuo por defecto
    if (caps.focusMode && caps.focusMode.includes('continuous')) {
      refAutofocoOn = true;
      await track.applyConstraints({ advanced: [{ focusMode: 'continuous' }] }).catch(() => {});
    }
    refActualizarBotonFoco();

    // Detectar cámaras traseras disponibles (para cambiar a gran angular)
    try {
      const devices = await navigator.mediaDevices.enumerateDevices();
      refCamaras = devices.filter(d => d.kind === 'videoinput');
      const btnCam = document.getElementById('ref-cam-btn');
      if (btnCam) btnCam.style.display = refCamaras.length > 1 ? '' : 'none';
    } catch(e) {}

    // Linterna disponible?
    if (!caps.torch) {
      const tb = document.getElementById('ref-torch-btn');
      if (tb) tb.style.opacity = '0.35';
    }
  } catch(e) { console.warn('refConfigurarCamara:', e); }
}

function refActualizarZoomLabel() {
  const lbl = document.getElementById('ref-zoom-label');
  if (!lbl) return;
  if (refZoomMax <= refZoomMin) { lbl.textContent = '1×'; return; }
  const factor = (refZoomActual / refZoomMin).toFixed(1);
  lbl.textContent = factor + '×';
}

// ── Zoom real ──
async function refCamZoom(dir) {
  if (!refCamTrack) return;
  const caps = refCamTrack.getCapabilities ? refCamTrack.getCapabilities() : {};
  if (!caps.zoom) { showToast('Zoom no disponible en esta cámara'); return; }

  const step = (refZoomMax - refZoomMin) / 10;
  const nuevo = Math.min(refZoomMax, Math.max(refZoomMin, refZoomActual + dir * step));

  if (nuevo === refZoomActual) return;
  refZoomActual = nuevo;

  try {
    await refCamTrack.applyConstraints({ advanced: [{ zoom: refZoomActual }] });
    refActualizarZoomLabel();
    if (navigator.vibrate) navigator.vibrate(20);
  } catch(e) {
    showToast('No se pudo aplicar el zoom');
  }
}

// ── Toggle de autofoco ──
async function refToggleAutofoco() {
  if (!refCamTrack) return;
  const caps = refCamTrack.getCapabilities ? refCamTrack.getCapabilities() : {};
  if (!caps.focusMode) { showToast('Control de enfoque no disponible'); return; }

  refAutofocoOn = !refAutofocoOn;

  try {
    if (refAutofocoOn) {
      await refCamTrack.applyConstraints({ advanced: [{ focusMode: 'continuous' }] });
      showToast('Autofoco activado');
    } else {
      // Fijar el enfoque actual
      if (caps.focusMode.includes('manual')) {
        await refCamTrack.applyConstraints({ advanced: [{ focusMode: 'manual' }] });
        showToast('Enfoque fijado');
      } else if (caps.focusMode.includes('single-shot')) {
        await refCamTrack.applyConstraints({ advanced: [{ focusMode: 'single-shot' }] });
        showToast('Enfoque fijado');
      }
    }
    refActualizarBotonFoco();
    ...
    showToast('No se pudo cambiar el enfoque');
  }
}

function refActualizarBotonFoco() {
  const b = document.getElementById('ref-focus-btn');
  if (!b) return;
  if (refAutofocoOn) {
    b.style.background  = 'var(--green-lt)';
...
// ── Cambiar entre cámaras traseras (gran angular / teleobjetivo) ──
async function refCambiarCamara() {
  if (refCamaras.length < 2) { showToast('Solo hay una cámara disponible'); return; }
  refCamaraIdx = (refCamaraIdx + 1) % refCamaras.length;
  const deviceId = refCamaras[refCamaraIdx].deviceId;

  showToast('Cambiando de cámara…');

  // Reiniciar Quagga con la nueva cámara
  try { Quagga.stop(); } catch(e) {}
  await new Promise(r => setTimeout(r, 200));

  Quagga.init(refQuaggaConfig({ deviceId: { exact: deviceId } }), err => {
    if (err) { showToast('⚠ No se pudo cambiar de cámara'); return; }
    Quagga.start();
    setTimeout(() => refConfigurarCamara(), 500);
    refBindDetectores();
  });
}

// ── Linterna ──
async function refToggleTorch() {
  if (!refCamTrack) return;
  const caps = refCamTrack.getCapabilities ? refCamTrack.getCapabilities() : {};
  if (!caps.torch) { showToast('Linterna no disponible en este dispositivo'); return; }
  refTorchOn = !refTorchOn;
  try {
    await refCamTrack.applyConstraints({ advanced: [{ torch: refTorchOn }] });
    const tb = document.getElementById('ref-torch-btn');
    if (tb) {
      tb.style.background = refTorchOn ? '#FEF08A' : 'var(--bg)';
      tb.style.color      = refTorchOn ? '#854D0E' : 'var(--slate-mid)';
    }
  } catch(e) { showToast('No se pudo activar la linterna'); }
}
...
function refDetenerScanner() {
  if (refScannerActivo) {
    ...
    try { Quagga.offProcessed(); Quagga.offDetected(); Quagga.stop(); } catch(e) {}
  ...
  const zl = document.getElementById('ref-zoom-label');
